...
**Date:** 2026-06-12 (CyclesEvidence source emission, reserve endpoint — WIP on `feat/evidence-emit-reserve`, the producer half of the dedicated-channel evidence emitter (long-pole gating `aeoess/agent-passport-system#43` (b), `cycles-protocol#103`, and the `replay_class` decision_replay→full_replay promotion). Adds `EvidenceQueueRepository` (LPUSH a serialized SOURCE record onto a dedicated Redis queue `evidence:pending`, key configurable via `cycles.evidence.queue.pending-key`) and `EvidenceEmitter` (`@Service`, fire-and-forget on a daemon executor mirroring `EventEmitterService`; stamps ONLY operational facts — `artifact_type`, `issued_at_ms` (at RESPONSE time = the authoritative event time, matching the reference fixtures, not async sign-time), `trace_id`, and the caller-supplied artifact-specific payload body. The server identity (`server_id`/`signer_did`) is added by the event-tier signing worker, co-located with the key). The `emit` API takes the payload body (not a fixed request/response pair) because the shape varies by artifact: `reserve`/`decide` = `{request,response}`, `commit`/`release` add `reservation_id`, `error` = `{endpoint,http_status,request,response}`. Wired into `ReservationController.create` — covers ALLOW and DENY (each is a `reserve` artifact carrying its decision), building `payload.reserve = {request,response}` via a null-safe LinkedHashMap. Deliberately a dedicated channel, NOT the webhook event stream: full lifecycle responses are an evidence concern, produced for every call regardless of webhook subscriptions. No wire change to existing endpoints (additive, async, non-blocking); no spec change (`cycles-evidence-v0.1` is schema-only and already authoritative). 3 new tests (`EvidenceEmitter` ×2 + `EvidenceQueueRepository` LPUSH, mocked Jedis — covers the thin repo for the 95% gate); `ReservationControllerTest` gains `@MockitoBean EvidenceEmitter` (existing tests unaffected — api module 0 failures/0 errors). FAN-OUT to decide/commit/release + the `error` artifact lands in follow-ups; the consumer/signer worker lives in cycles-server-events. Review-2: documented the async-serialization CONTRACT on `EvidenceEmitter.emit` — `payloadBody` is serialized on the executor thread, so callers must pass an effectively-immutable snapshot; holds for the Spring MVC controllers whose per-request DTOs are not mutated after return.), 2026-05-22 (v0.1.25.21 — `expires_from`/`expires_to` and `finalized_from`/`finalized_to` ISO-8601 time-window filters on `GET /v1/reservations` per `cycles-protocol-v0.yaml` revision 2026-05-22 (runcycles/cycles-protocol#98); closes runcycles/cycles-server#162. Four new query params mirroring the v0.1.25.20 `from`/`to` shape: `expires_*` binds to `expires_at_ms` (required field, every row), `finalized_*` binds to `finalized_at_ms` (populated only on COMMITTED/RELEASED; ACTIVE and EXPIRED normatively excluded). Three windows compose with AND. `finalized_at_ms` added as an optional field on `ReservationSummary` so clients filtering with `finalized_*` can see the timestamp without a follow-up `getReservation` — strict-schema-compatible because the field is `@JsonInclude(NON_NULL)`. `FilterHasher` extends with four more `Long` args (10 → 14) using independent gated emission per pair — preserves byte-exact back-compat for v0.1.25.18 cursors (golden `2f397ea0e8fb53b7`) AND v0.1.25.20 cursors with from/to set (golden `ad7204d521cfd133`). `RedisReservationRepository.listReservations` signature 14 → 18 args. Two new predicate helpers (`expiresAtInWindow`, `finalizedAtInWindow`) applied in both legacy SCAN-cursor and sorted paths. Validation: each new pair `from > to` → 400; malformed values → 400 with distinct per-param message; blank strings treated as unset. 557 tests pass (384 data + 173 api), +19 vs v0.1.25.20.),
